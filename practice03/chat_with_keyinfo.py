...
def extract_key_information(messages, client):
    """Extract key information from chat history using 5W rule"""
    # Prepare key information extraction prompt
    keyinfo_prompt = """
    Please extract key information from the following chat history using the 5W rule:
    - Who: Who is involved in the conversation?
    - What: What happened or was discussed?
    - When: When did this happen? (if mentioned)
    - Where: Where did this happen? (if mentioned)
    - Why: Why did this happen? (if mentioned)
    
    Extract multiple key information items if there are several topics discussed.
    Format your response as a list of key information items, each with clear 5W elements.
    ...
    # Create key information extraction request
    keyinfo_messages = [
        {
            "role": "system",
            "content": "You are a helpful assistant that extracts key information from chat history using the 5W rule. Be concise and clear, focusing on the most important information."
        },
        {
            "role": "user",
            "content": keyinfo_prompt + "\n".join(relevant_messages)
        }
    ]
    
    # Get key information from LLM
    print("\n=== Extracting key information ===")
    keyinfo_result = client.chat(keyinfo_messages)
    keyinfo = keyinfo_result['content']
    print(f"=== Key information extracted ===\n{keyinfo}\n")
    
    return keyinfo


def save_key_information(keyinfo):
    """Save key information to local file"""
    log_dir = "D:\\chat-log"
...
def search_chat_history(query):
    """Search chat history from log file"""
    log_dir = "D:\\chat-log"
    log_file = os.path.join(log_dir, "log.txt")
    
    # Check if log file exists
    if not os.path.exists(log_file):
        return {"error": "Chat history log file not found"}
...
def main():
    """Main function for interactive chat with tool calling, chat history summarization, and key information extraction"""
    print("=== Interactive LLM Chat with Tool Calling, Summary, and Key Info Extraction ===")
...
7. search_chat_history(query: str)
   - 搜索聊天历史记录
   - 参数: query (搜索查询)
   - 当用户发送的信息用"/search"开头，或用户表达了"查找聊天历史"这个意思，或你认为应该查找聊天历史时使用此工具
...
9. When users send messages starting with "/search" or express the need to find chat history, use the search_chat_history tool to help them.
"""
        ...
        while True:
            ...
            if current_rounds >= 5 or context_length > 3000:
            ...
            # Check if we need to extract key information (every 5 rounds)
            if current_rounds > 0 and current_rounds % 5 == 0:
                print("\n=== Extracting key information from chat history ===")
                # Extract key information using 5W rule
                keyinfo = extract_key_information(chat_history, client)
                # Save key information to local file
                save_key_information(keyinfo)
                print("=== Key information extraction completed ===\n")
